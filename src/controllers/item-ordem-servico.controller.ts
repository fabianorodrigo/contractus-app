...
export class ItemOrdemServicoController {
    ...
    async count(@param.where(ItemOrdemServico) where?: Where<ItemOrdemServico>): Promise<Count> {
    ...
    async find(@param.filter(ItemOrdemServico) filter?: Filter<ItemOrdemServico>): Promise<ItemOrdemServico[]> {
